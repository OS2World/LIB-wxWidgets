...
$(OBJS)\except.exe :  $(EXCEPT_OBJECTS)
	...
	@%append $(OBJS)\except.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(EXCEPT_OBJECTS)) do @%append $(OBJS)\except.lbc file %i
	@for %i in ( $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\except.lbc library %i
	@%append $(OBJS)\except.lbc
	@for %i in () do @%append $(OBJS)\except.lbc option stack=%i
	wlink @$(OBJS)\except.lbc

$(OBJS)\except_except.obj :  .AUTODEPEND .\except.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(EXCEPT_CXXFLAGS) $<
