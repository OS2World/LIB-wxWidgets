...
$(OBJS)\zip.exe :  $(ZIP_OBJECTS)
	@%create $(OBJS)\zip.lbc
	@%append $(OBJS)\zip.lbc option quiet
	@%append $(OBJS)\zip.lbc name $^@
	@%append $(OBJS)\zip.lbc option caseexact
	@%append $(OBJS)\zip.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(ZIP_OBJECTS)) do @%append $(OBJS)\zip.lbc file %i
	@for %i in ( $(__WXLIB_HTML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\zip.lbc library %i
	@%append $(OBJS)\zip.lbc
