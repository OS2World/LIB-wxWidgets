...
$(OBJS)\sound.exe :  $(SOUND_OBJECTS)
	@%create $(OBJS)\sound.lbc
	@%append $(OBJS)\sound.lbc option quiet
	@%append $(OBJS)\sound.lbc name $^@
	@%append $(OBJS)\sound.lbc option caseexact
	@%append $(OBJS)\sound.lbc  $(__DEBUGINFO_1)  libpath $(LIBDIRNAME) system os2v2_pm $(LDFLAGS)
	@for %i in ($(SOUND_OBJECTS)) do @%append $(OBJS)\sound.lbc file %i
	@for %i in ( $(__WXLIB_ADV_p)  $(__WXLIB_HTML_p)  $(__WXLIB_XML_p)  $(__WXLIB_CORE_p)  $(__WXLIB_BASE_p)  $(__WXLIB_MONO_p) $(__LIB_TIFF_p) $(__LIB_JPEG_p) $(__LIB_PNG_p)  wxzlib$(WXDEBUGFLAG).lib  wxregex$(WXUNICODEFLAG)$(WXDEBUGFLAG).lib wxexpat$(WXDEBUGFLAG).lib $(EXTRALIBS_FOR_BASE) upm32.lib) do @%append $(OBJS)\sound.lbc library %i
	@%append $(OBJS)\sound.lbc
...
$(OBJS)\sound_sound.obj :  .AUTODEPEND .\sound.cpp
	$(CXX) -bt=os2 -zq -fo=$^@ $(SOUND_CXXFLAGS) $<
